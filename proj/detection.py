...
#без данного вызова не удавалось загрузить видео
set_ssl()
...
best_quality_video = load_video_from_youtube(url)

#получаем размер кадра
frame_x_pix, frame_y_pix = (int(x) for x in re.findall(r'\d+', best_quality_video.resolution))
...
capture = cv2.VideoCapture(best_quality_video.url)
# main loop
